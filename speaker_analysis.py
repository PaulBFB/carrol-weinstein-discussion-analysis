...
def analyze_speaker_data(csv_file_path: pathlib.Path):
    ...
    logger.info(
        "Included education levels: Undergraduate, Graduate, High School/Undergraduate"
    )
    logger.info("=" * 60)
    ...
    for speaker in speakers:
        ...
        unique_segments = speaker_data.groupby(["start_time", "end_time"])[
            "duration"
        ].first()
        ...
        if len(sophisticated_concepts) > 0:
            sophistication_scores = [
                education_hierarchy.get(level, 0)
                for level in sophisticated_concepts["education_level"]
            ]
        ...
        results[speaker] = {
            ...
            "avg_segment_duration": total_duration / total_unique_segments
            if total_unique_segments > 0
            else 0,
            ...
            "concept_density": total_concepts / total_duration
            if total_duration > 0
            else 0,
        ...
        logger.info("-" * 40)
        logger.info(
            f"Speaking Time: {total_duration:.2f} seconds ({total_duration / 60:.2f} minutes)"
        )
        logger.info(f"Number of Unique Speaking Segments: {total_unique_segments}")
        logger.info(
            f"Average Segment Duration: {total_duration / total_unique_segments:.2f} seconds"
        )
        ...
        if total_concepts > 0:
            logger.info(
                f"  • Concept Density: {total_concepts / total_duration:.4f} concepts per second"
            )
            logger.info(
                f"  • Average Sophistication Score: {avg_sophistication:.2f}/4.0"
            )
            ...
            for level, count in education_counts.items():
                ...
                logger.info(
                    f"  • {level}: {count} concepts ({percentage:.1f}%) [Score: {score}]"
                )
...
def create_visualizations(results, save_plot: bool = True):
    """
    ...
    fig.suptitle(
        "Speaker Analysis Dashboard - Sophisticated Concepts Only",
        fontsize=16,
        fontweight="bold",
    )
    ...
    speaking_times = [
        results[speaker]["total_speaking_time"] / 60 for speaker in speakers
    ]  # Convert to minutes
    axes[0, 0].bar(
        speakers, speaking_times, color=["skyblue", "lightcoral", "lightgreen"]
    )
    ...
    total_concepts = [
        results[speaker]["total_sophisticated_concepts"] for speaker in speakers
    ]
    unique_concepts = [
        results[speaker]["unique_sophisticated_concepts"] for speaker in speakers
    ]
    ...
    axes[1, 0].bar(
        x - width / 2, avg_scores, width, label="Average Score", color="gold", alpha=0.8
    )
    axes[1, 0].bar(
        x + width / 2, max_scores, width, label="Max Score", color="crimson", alpha=0.8
    )
    ...
    for i, v in enumerate(concept_density):
        if v > 0:
            axes[1, 1].text(
                i, v + max(concept_density) * 0.01, f"{v:.4f}", ha="center", va="bottom"
            )
...
def generate_summary_report(results):
    ...
    max_time_speaker = max(
        results.keys(), key=lambda x: results[x]["total_speaking_time"]
    )
    max_time = results[max_time_speaker]["total_speaking_time"]

    # Find speaker with highest concept density (among those with concepts)
    speakers_with_concepts = {
        k: v for k, v in results.items() if v["concept_density"] > 0
    }

    if speakers_with_concepts:
        max_density_speaker = max(
            speakers_with_concepts.keys(), key=lambda x: results[x]["concept_density"]
        )
        max_density = results[max_density_speaker]["concept_density"]

        # Find speaker with highest average sophistication
        max_soph_speaker = max(
            speakers_with_concepts.keys(),
            key=lambda x: results[x]["avg_sophistication_score"],
        )
        max_soph = results[max_soph_speaker]["avg_sophistication_score"]

        logger.info(f"🎯 KEY INSIGHTS:")
        logger.info(
            f"  • Most Speaking Time: {max_time_speaker} ({max_time / 60:.1f} minutes)"
        )
        logger.info(
            f"  • Highest Sophisticated Concept Density: {max_density_speaker} ({max_density:.4f} concepts/sec)"
        )
        logger.info(
            f"  • Most Sophisticated Concepts: {max_soph_speaker} (avg score: {max_soph:.2f}/4.0)"
        )
    else:
        logger.info(f"🎯 KEY INSIGHTS:")
        logger.info(
    ...
    total_concepts = sum(
        results[speaker]["total_sophisticated_concepts"] for speaker in results
    )
    total_unique_concepts = len(
        set().union(
            *[set(results[speaker].get("concept_list", [])) for speaker in results]
        )
    )

    logger.info(f"\n📊 OVERALL STATISTICS:")
    logger.info(
        f"  • Total Discussion Time: {total_time / 60:.1f} minutes (deduplicated)"
    )
    ...
    if total_time > 0 and total_concepts > 0:
        logger.info(
            f"  • Average Sophisticated Concepts per Minute: {total_concepts / (total_time / 60):.1f}"
        )
    ...
    logger.info(
        f"   Only Undergraduate, Graduate, and High School/Undergraduate levels included"
    )
